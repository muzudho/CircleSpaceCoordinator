...
`bindings.layout` はスラッシュ区切りの絶対パス（例: `/regularPanel/page`）を指定します。同じレイアウトツリーの binding は `parentModel` を揃え、通常表示なら `/events/regular`、コンパクト表示なら `/events/compact` を基準に、`body/actions/open` などの子孫へ接続します。子レイアウトが使うセルにモデルも重ねて配置するとエラーになります。`body`・`actions`・行の `item` の領域は、アプリが対応する子レイアウトの計算結果から取得します。
