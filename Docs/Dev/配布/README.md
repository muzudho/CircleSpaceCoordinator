...
- **他人が作った DLL・EXE には自己署名を付けません。** Debug / F5、Release、publish のすべてに適用します。外部ライブラリは提供元の署名・未署名状態を維持し、署名対象はこのリポジトリで作成するファイルの明示的な許可リストに限定します。[署名対象の方針](コード署名.md#署名対象の方針) を参照してください。
...
| 文書 | 内容 |
| --- | --- |
| [v1.0.0 の ZIP 起動と Smart App Control](v1.0.0のZIP起動とSmart%20App%20Control.md) | 起動成功の報告、設定・ログの確認結果、未署名でも許可され得る理由 |
