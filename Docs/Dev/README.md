...
| [スタイル設定の実装指針](StyleSettings/README.md) | イベント一覧の導入で得た知見、原本管理・埋め込み・監視、別ページへの展開手順 |
| [設計](設計/) | 各機能とデータ形式の設計 |
| [マルチユーザー共同編集計画](設計/マルチユーザー共同編集計画.md) | パッケージ項目ごとの変更タグ、１行チェンジログ必須入力、画面終了時の検証 |
| [プロジェクトのポータブル計画](設計/プロジェクトのポータブル計画.md) | 選択式の部分保存・受渡し、知見のライブラリー化の構想 |
| [プロジェクトのポータブル実装計画](開発/プロジェクトのポータブル実装計画.md) | 既存実装の入口、段階別タスク、互換性と受入確認 |
| [フレーム配置ポータブル計画](設計/フレーム配置ポータブル計画.md) | 担当者間の受渡し、会場名、配置図側の申込定義、マル秘警告の実装計画 |
