...
GUI起動時は編集エンジンだけを非表示で起動する。編集エンジンに `--thinking-directory` と `--thinking-log-directory` を渡し、思考エンジンは自動最適化（単発API・ジョブ）または空席配置の実行時に起動する。動的なループバックポートで接続し、結果の取得・反映後に子プロセスを終了する。ジョブの完了通知は子プロセス終了後に出すため、続けて再実行できる。

思考エンジンの起動は30秒でタイムアウト。ジョブの起動待ち中の停止要求では起動をキャンセルして後始末する。計算開始後の停止は従来どおり最高案を受け取って終了する。外部サーバーを `--thinking-address` で指定する構成は維持し、そのプロセスを起動・終了しない。

所有関係はGUI→編集エンジン→思考エンジン。通常のGUI終了では所有する編集エンジンのプロセスツリーを終了する。両エンジンはホスト構築前から親の終了を監視し、親が終了した場合は自分も終了する。編集エンジンのホスト停止時にも実行中の思考プロセスを終了する。ページ移動によるプロセス管理ではなく、計算処理の利用期間で管理する。
