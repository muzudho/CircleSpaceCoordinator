...
internal static class Program
{
    ...
    private static void DialogValidation()
    {
        ...
        foreach (var value in new[] { "", " ", "0", "1", "-1", (-0.001m).ToString(), (-0.375m).ToString(), 0.001m.ToString(), 0.123m.ToString(), 1.000m.ToString() }) AssertEqual<string?>(null, EditorDialogValidation.Weight(value));
        foreach (var value in new[] { "NaN", (-1.001m).ToString(), (-0.0001m).ToString(), "2", "1e-3", "文字", 0.1234m.ToString(), 1.001m.ToString(), 0.0001m.ToString() }) AssertEqual(true, EditorDialogValidation.Weight(value) is not null);
        ...
        AssertEqual(true, EditorDialogValidation.TryParseWeight((-0.375m).ToString(), out var negativeWeight));
        AssertEqual(-0.375m, negativeWeight);
