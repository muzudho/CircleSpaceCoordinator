...
public sealed partial class VenueEditorGame
{
    ...
    private bool CanShowEditorHover => IsActive && modalDialog is null && !modalInputDrain && !projectMenuOpen && !projectMenuDrain && !toolRingOpen && !toolRingInputDrain && !spaceCatalogOpen && !spaceCatalogDrain;
    ...
    private void DrawModalDialog()
    {
        ...
        for (var index = 0; index < modalButtons.Count; index++)
        {
            ...
            if (packageReadDialog is { CanOperate: false } && (modalButtons[index].Action == ModalDialogAction.Stop || modalButtons[index].Action == PackageRenameAction))
                button.IsEnabled = false;
