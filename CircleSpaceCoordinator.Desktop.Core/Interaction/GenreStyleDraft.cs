...
/// <summary>Edits genre appearances without changing the project until explicitly applied.</summary>
public sealed class GenreStyleDraft
{
    ...
    private readonly List<GenreStyleDefinition> rows;
    private readonly GenreStyleDefinition[] otherStyles;
    public IReadOnlyList<GenreStyleDefinition> Rows { get; }

    public GenreStyleDraft(CircleSpaceProject project)
    {
        var genres = project.Participants.Select(item => item.GenreId).Where(id => !string.IsNullOrWhiteSpace(id))
            .Select(id => id!).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal).ToArray();
        var configured = project.GenreStyles.ToDictionary(style => style.GenreId, StringComparer.Ordinal);
        rows = genres.Select((id, index) =>
        {
            var patternIndex = index / 12;
            var style = configured.GetValueOrDefault(id) ?? new GenreStyleDefinition(id, Colors[index % 12].Id, "white",
        ...
        otherStyles = project.GenreStyles.Where(style => !genres.Contains(style.GenreId, StringComparer.Ordinal)).ToArray();
    }
    ...
    public GenreStyleDefinition[] Build()
    {
        foreach (var style in rows)
        {
            foreach (var color in new[] { style.PrimaryColor, style.SecondaryColor })
                if (!Colors.Any(choice => choice.Id == color) && !RgbHexColor.TryParse(color, out _, out _, out _))
                    throw new InvalidDataException($"{style.GenreId} の色「{color}」を色名または #RRGGBB で指定してください。");
            if (!Patterns.Any(choice => choice.Id == style.Pattern))
                throw new InvalidDataException($"{style.GenreId} の網掛けパターンを選択してください。");
        }
